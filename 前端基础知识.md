...
align-items定义了元素在交叉轴上的对齐方式，align-items可以取5个值：
...
align-content定义了元素在多根轴线上的对齐方式，一根轴线时，不起作用。

align-content和align-items比较容易混淆，首先两者都是定义交叉轴上的排列方式，align-items是针对每一行的元素做排列处理，比如align-items:center是对每一行的元素在交叉轴垂直居中；align-content是对多行的元素在交叉轴上居中。
...
上面的都是容器的属性，item的属性可以设置一下6个：

###2.1.7 order

定义的是元素的排列顺序。数值越小越靠前，默认是0
